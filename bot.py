...
CARD_ID_SYSTEM_PROMPT = """画像からポケモンカード（またはトレーディングカード）の情報を読み取り、必ずJSON形式のみで返してください。説明文は不要です。

{
  "card_name": "カード名（日本語または英語）",
  "set_name": "セット名またはシリーズ名（不明なら空文字）",
  "grade": "グレード（例: PSA10, PSA9, BGS9.5, RAW, 不明）"
}

- PSAスラブの場合はラベルの数字を読み取ること
- カードが特定できない場合: {"error": "カードを識別できませんでした"}
- グレーディングされていない生カードの場合: grade は "RAW" とする"""

PSA_PRICE_SYSTEM_PROMPT = """指定されたポケモンカードのグレード品について、メルカリ・スニダン・eBayの直近の取引価格をウェブ検索で調べてください。
...
注意事項:
- 必ず「売り切れ」「Sold」の実際の取引価格を探すこと（出品中の価格ではない）
